...
TContainer::TContainer(QWidget *parent, QPoint p, QLabel* status_, QLabel* x_y_, QWidget *cWidget) : QWidget(parent) {
    mode = NONE;
    childWidget = cWidget;
...
void TContainer::keyPressEvent(QKeyEvent *e) {
    ...
    if (QApplication::keyboardModifiers() == Qt::ControlModifier) {
        ...
        move(newPos);
        x_y->setText(QString::number(newPos.x()) + QString(", ") + QString::number(newPos.y()));
    }
...
void TContainer::mouseMoveEvent(QMouseEvent *e) {
    ...
    if ((mode == MOVE || mode == NONE) && e->buttons() && Qt::LeftButton) {
        ...
        x_y->setText(QString::number(toMove.x()) + QString(", ") + QString::number(toMove.y()));
        emit newGeometry(this->geometry());
        this->parentWidget()->repaint();
        return;
    }
    if ((mode != MOVE) && e->buttons() && Qt::LeftButton) {
        ...
        case RESIZETL: {    //Left-Top
            ...
            QFont font;
            font.setPixelSize(this->geometry().width() > this->geometry().height() ? this->geometry().height()/3 : this->geometry().width()/3);
        ...
        case RESIZEBL: {    //Left-Bottom
            ...
            emit RESIZE();
            x_y->setText(QString::number(toMove.x()) + QString(", ") + QString::number(this->y()));
            break;
        }
        case RESIZEB: {     //Bottom
            resize(width(), e->y());
...
//            childWidget->
        ...
        case RESIZET: {     //Top
            ...
            move(this->x(), toMove.y());
            emit RESIZE();
            x_y->setText(QString::number(this->y()) + QString(", ") + QString::number(toMove.y()));
            break;
        }
        case RESIZER: {     //Right
            resize(e->x(), height());
